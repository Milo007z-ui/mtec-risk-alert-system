...
#
# dynaudnorm      คำนวณเกนแยกทีละเฟรมสั้น ๆ (ms) แทนการ "ไต่ระดับ" ทั้งไฟล์แบบ speechnorm
#                 ที่ลองก่อนหน้านี้แล้วได้แค่ ~2 dB เพราะไฟล์สั้น 6-8 วิ ไต่ไม่ทันถึงเป้า
#                 เปรียบเหมือนเร่งเครื่องแบบค่อยเหยียบ แต่ทางวิ่งสั้นเกินไป
#   f=100         ขนาดเฟรม 100 ms เล็กพอที่จะตอบสนองไวภายในไฟล์สั้น
#   g=15          จำนวนเฟรมที่ใช้ปรับเกนให้ลื่น (ต้องเป็นเลขคี่) กันเกนกระโดดฮวบฮาบ
#   p=0.95        เป้าหมายความดังสูงสุดหลังปรับ (95% ของเพดาน) เผื่อไว้ให้ alimiter อีกชั้น
#   m=15          เพดานเกนสูงสุดที่จะขยายได้ (15 เท่า) กันขยายจนได้ยินเป็นเสียงฮืดฮาดพื้นหลัง
# alimiter        กันยอดทะลุเป็นด่านสุดท้าย level=disabled กันไม่ให้มันไป normalize ซ้ำ
FILTER = "highpass=f=250,dynaudnorm=f=100:g=15:p=0.95:m=15,alimiter=limit=0.97:level=disabled"
