...
RULES = [
    (
        ...
        lambda p: (
            ...
            if p["level"] == "high"
            else "ใช้ความเร็วให้เหมาะสม และใช้ความระมัดระวัง"
        ),
    ),
    (
        "junction",
        ...
        lambda p: "ใช้ความเร็วให้เหมาะสม และระวังรถตัดผ่านทางแยก",
    ),
    (
        "u-turn",
    ...
    (
        ...
        lambda p: "ใช้ความเร็วให้เหมาะสมก่อนเข้าโค้ง และงดแซงในช่วงนี้",
    ),
    (
        "access-road",
...
def build_alert_message(point, distance_m):
    ...
    if point["level"] == "high":
        advice = top["advice"] if top else "ใช้ความเร็วให้เหมาะสม และใช้ความระมัดระวังเป็นพิเศษ"
        return f"ข้างหน้าอีก {dist} เมตร ใกล้จุดเสี่ยงสูง โปรด{advice}"
    if point["level"] == "medium":
        advice = top["advice"] if top else "ใช้ความเร็วให้เหมาะสม และขับขี่ด้วยความระมัดระวัง"
