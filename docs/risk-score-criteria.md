# เกณฑ์การให้คะแนนจุดเสี่ยง — โมเดล v2 (4 เกณฑ์ × 25 คะแนน, ตารางเกณฑ์ + จุดตัดคงที่)
...
|---|---|---|---|
| การให้คะแนน | log-ratio-to-max | **ตารางเกณฑ์ (banded rubric)** ตั้งแต่ r4 — r1–r3 ใช้ Percentile Rank | อ่านค่าดิบเทียบตารางได้ตรงๆ ไม่ต้องเทียบจุดอื่น และยังทนต่อ outlier เพราะช่วงบนสุดเป็นปลายเปิด |
...
| เกณฑ์ลักษณะถนน | ยืมสัดส่วน conflict point ต่างประเทศ | **% เหตุที่เกิดบริเวณจุดตัดกระแสจราจร** (ตั้งแต่ r4) | อ่านเข้าใจทันที ("30% ของเหตุเกิดที่ทางแยก/ทางโค้ง") แทน gc_score ทศนิยม 1.0151 ที่อธิบายยาก |
| แบ่งระดับ | mean±0.5SD + KSI override | **จุดตัดคงที่ 3 ชั้น** (≤40/≤60/>60) | อธิบายง่ายสำหรับนำเสนอ (v2568-r4) — เดิม r1/r2 ใช้ Jenks Natural Breaks (ยังเก็บฟังก์ชันไว้อ้างอิง ดู [ข้อจำกัด](#ข้อจำกัด-ควรระบุในรายงาน)) |
...
**ห้าม**คำนวณค่าอ้างอิง (ตารางเกณฑ์, จุดตัดระดับ, FI weights) ใหม่แบบ dynamic
...
2. ถึงรอบจึงรัน `scripts/build_risk_points.py` ใหม่ แล้ว **ล็อกผลเป็นเวอร์ชัน**
   (ปัจจุบัน `v2568-r4` — snapshot ทุกรอบเก็บไว้ที่ `data/calibrations/`)
...
ผลรอบ v2568-r4: 4,460 เหตุการณ์มีพิกัด → **171 จุดเสี่ยง** (ครอบคลุม 3,784 เหตุการณ์,
...
## วิธีให้คะแนนทุกเกณฑ์: ตารางเกณฑ์ (Banded Rubric) — ตั้งแต่ v2568-r4

เทียบค่าดิบกับตาราง แล้วอ่านคะแนนออกมาตรงๆ เกณฑ์ละ **5 / 10 / 15 / 20 / 25 คะแนน**
(ช่วงบนสุดเป็นปลายเปิด) — ไม่ต้องคำนวณเทียบกับจุดอื่น ไม่ต้องรู้ข้อมูลทั้งชุด

| เกณฑ์ | 5 | 10 | 15 | 20 | 25 |
|---|---|---|---|---|---|
| 1. ความถี่ (ครั้ง) | ≤3 | ≤5 | ≤10 | ≤20 | >20 |
| 2. มูลค่าความเสียหาย (บาท) | ≤100,000 | ≤500,000 | ≤2,000,000 | ≤10,000,000 | >10,000,000 |
| 3. Single-Vehicle Ratio (%) | ≤20 | ≤35 | ≤50 | ≤70 | >70 |
| 4. จุดตัดกระแสจราจร (%) | 0 | ≤5 | ≤15 | ≤30 | >30 |

**ยังทนต่อ outlier:** ช่วงบนสุดเป็นปลายเปิด จุดที่มี 994 ครั้ง กับจุดที่มี 21 ครั้ง
ได้ 25 คะแนนเท่ากัน — outlier จึงไม่ดึงคะแนนของจุดอื่นให้เพี้ยน (ต่างจาก min-max/
log-ratio ที่ทดสอบแล้วพัง ดูตารางบนสุด)

**ที่มาของขีดแบ่ง** (design choice อิงการกระจายจริงของข้อมูลรอบนี้ + ความหมายเชิงวิศวกรรม):
- ความถี่: เริ่มที่ 3 ครั้งตามนิยาม Black Spot ขั้นต่ำของ DBSCAN
- ความเสียหาย: 2 ล้าน = ผู้บาดเจ็บสาหัส 1 ราย, 10 ล้าน > ผู้เสียชีวิต 1 ราย (TDRI 6.7 ล้าน)
- Single-Vehicle: 50% = จุดเปลี่ยนระหว่างปัญหารถเสียหลักเดี่ยว vs ปัญหาความขัดแย้งจราจร
- จุดตัดกระแส: 0% = ทางตรงล้วน (69% ของจุดทั้งหมดอยู่กลุ่มนี้)

> **วิธีเดิม (r1–r3): Percentile Rank** — `s.rank(pct=True, method='average') * 100`
> อ้างอิง Nardo et al. (2008) OECD/JRC Handbook: *"Ranking is the simplest normalisation
> technique. This method is not affected by outliers..."* — ยังเก็บฟังก์ชัน
> `percentile_rank()` ไว้ในสคริปต์พร้อม self-test เพื่อเปรียบเทียบวิธี
> เปลี่ยนออกเพราะอธิบายยากเกินไปสำหรับการนำเสนอ (ต้องเข้าใจว่าคะแนน = อันดับเทียบจุดอื่น)

## เกณฑ์ที่ 1: ความถี่อุบัติเหตุ (เต็ม 25 คะแนน)

```
Frequency_Score(จุด) = คะแนนจากตาราง(จำนวนอุบัติเหตุทั้งหมดที่จุดนั้น)  -> 5/10/15/20/25
```
...
## เกณฑ์ที่ 2: มูลค่าความเสียหายทางเศรษฐกิจ (เต็ม 25 คะแนน)

```
economic_loss(จุด) = ผู้เสียชีวิต×6,700,000 + ผู้บาดเจ็บสาหัส×2,000,000 + ผู้บาดเจ็บเล็กน้อย×58,000  (บาท)
EconomicLoss_Score(จุด) = คะแนนจากตาราง(economic_loss)  -> 5/10/15/20/25
```
...
## เกณฑ์ที่ 3: Single vs Multiple Vehicle Crash Ratio (เต็ม 25 คะแนน)
...
SingleVehicle_Score(จุด) = คะแนนจากตาราง(Single-Vehicle Ratio)  -> 5/10/15/20/25
```
...
## เกณฑ์ที่ 4: สัดส่วนเหตุที่จุดตัดกระแสจราจร (เต็ม 25 คะแนน)
...
Geometry_Score(จุด) = คะแนนจากตาราง(conflict_pct)  -> 5/10/15/20/25
```
...
รอบ r1–r3 ใช้ค่านี้ถ่วงน้ำหนักเป็น `gc_score` ตั้งแต่ r4 เก็บไว้เป็นหลักฐานสนับสนุน
...
```
Risk Score(จุด) = Frequency + EconomicLoss + SingleVehicle + Geometry     (เต็ม 100)
                  (เต็ม 25)   (เต็ม 25)      (เต็ม 25)       (เต็ม 25)
```
...
**แบ่ง 3 ระดับด้วยจุดตัดคงที่** (ตั้งแต่รอบ v2568-r4): ≤40 ต่ำ, 40–60 ปานกลาง, >60 สูง
...
ผลรอบ **v2568-r4** (ค่าคงที่ ไม่ต้องคำนวณใหม่แม้ Risk Score เปลี่ยนตามข้อมูล):
...
Risk Score จริงกระจายอยู่ในช่วง 25–80 (เป็นพหุคูณของ 5 เสมอ)

## ข้อจำกัด (ควรระบุในรายงาน)

1. **คะแนนหยาบ (ผลจากการใช้ตารางเกณฑ์ตั้งแต่ r4)** — คะแนนรวมเป็นไปได้เพียง 12 ค่า
   (25, 30, ... 80) ทำให้มีจุดจำนวนมากได้คะแนนเท่ากันเป๊ะ (สูงสุด 31 จุดที่คะแนน 50)
   จึงเรียงลำดับ "จุดไหนอันตรายกว่ากัน" ได้หยาบกว่า Percentile Rank เดิมที่ละเอียดถึงทศนิยม
2. **ขีดแบ่งช่วงในตารางทั้ง 16 ตัวเป็นดุลพินิจผู้ออกแบบ** (expert judgment) อิงการกระจาย
   จริงของข้อมูลรอบนี้ + ความหมายเชิงวิศวกรรม ไม่ใช่ค่าที่มีทฤษฎีกำหนดตายตัว —
   ถ้าถูกถามว่า "ทำไม 20 ครั้ง ไม่ใช่ 25" ต้องตอบตรงๆ ว่าเป็น design choice
3. จุดตัดระดับคงที่ 40/60 (ตั้งแต่ r3) เป็นการเลือกเชิงนำเสนอเช่นกัน ไม่ได้คำนวณจาก
   การกระจายของข้อมูลเหมือน Jenks Natural Breaks ที่ใช้ในรอบ r1–r2
4. เกณฑ์ Diverging มีตัวอย่างเพียง 54 เหตุการณ์ — FI weight 0.835 มีความไม่แน่นอนสูง
   (เป็นข้อมูลประกอบ ไม่กระทบคะแนนแล้วตั้งแต่ r4)
5. ข้อมูล 167 เหตุการณ์ไม่มีพิกัด (คิดเป็น 3.6%) ถูกตัดจากการจับกลุ่ม
   แต่ยังใช้ประมาณ FI Rate (ความรุนแรงเป็นข้อมูลจริงแม้ไม่มีพิกัด)
6. เหตุการณ์ noise 676 รายไม่เข้าจุดเสี่ยงใด — เป็นธรรมชาติของ DBSCAN
   (เหตุกระจัดกระจายไม่ถือเป็น hotspot) ควรรายงานตัวเลขนี้ควบคู่เสมอ
7. `รถที่เกิดเหตุ` นับจำนวนคัน ไม่แยกประเภทคู่กรณี (เช่น รถ-คนเดินเท้า นับเป็น 1 คัน
   จึงเข้ากลุ่ม Single)
8. 69% ของจุดมี `conflict_pct` = 0 (ทางตรงล้วน) เกณฑ์ที่ 4 จึงแยกความต่างได้น้อย
   ในทางปฏิบัติ — เป็นลักษณะของข้อมูลต้นทาง ไม่ใช่ข้อบกพร่องของสูตร
