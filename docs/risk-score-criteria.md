...
2. ถึงรอบจึงรัน `scripts/build_risk_points.py` ใหม่ แล้ว **ล็อกผลเป็นเวอร์ชัน**
   (ปัจจุบัน `v2568-r2` — snapshot ทุกรอบเก็บไว้ที่ `data/calibrations/`)
...
ผลรอบ v2568-r2: 4,460 เหตุการณ์มีพิกัด → **171 จุดเสี่ยง** (ครอบคลุม 3,784 เหตุการณ์,
...
— ผลจริงรอบ v2568-r2 ตรงกับค่าอ้างอิงในสเปก:
...
ผลรอบ **v2568-r2** (คำนวณจากข้อมูลจริง แล้วล็อกไว้ — ห้าม hardcode ถาวร
รอบถัดไปต้องคำนวณใหม่ตามตาราง):

| ระดับ | ช่วง Risk Score | จำนวนจุด |
|---|---|---|
| 🟢 ต่ำ | ≤ 41.4 | 53 |
| 🟠 ปานกลาง | 41.4 – 58.2 | 78 |
| 🔴 สูง | > 58.2 | 40 |
