...
| แบ่งระดับ | mean±0.5SD + KSI override | **จุดตัดคงที่ 3 ชั้น** (≤40/≤60/>60) | อธิบายง่ายสำหรับนำเสนอ (v2568-r3) — เดิม r1/r2 ใช้ Jenks Natural Breaks (ยังเก็บฟังก์ชันไว้อ้างอิง ดู [ข้อจำกัด](#ข้อจำกัด-ควรระบุในรายงาน)) |
...
2. ถึงรอบจึงรัน `scripts/build_risk_points.py` ใหม่ แล้ว **ล็อกผลเป็นเวอร์ชัน**
   (ปัจจุบัน `v2568-r3` — snapshot ทุกรอบเก็บไว้ที่ `data/calibrations/`)
...
ผลรอบ v2568-r3: 4,460 เหตุการณ์มีพิกัด → **171 จุดเสี่ยง** (ครอบคลุม 3,784 เหตุการณ์,
...
— ผลจริงรอบ v2568-r3 ตรงกับค่าอ้างอิงในสเปก:
...
**แบ่ง 3 ระดับด้วยจุดตัดคงที่** (ตั้งแต่รอบ v2568-r3): ≤40 ต่ำ, 40–60 ปานกลาง, >60 สูง
...
ผลรอบ **v2568-r3** (ค่าคงที่ ไม่ต้องคำนวณใหม่แม้ Risk Score เปลี่ยนตามข้อมูล):
...
3. เหตุการณ์ noise 676 รายไม่เข้าจุดเสี่ยงใด — เป็นธรรมชาติของ DBSCAN
...
6. จุดตัดระดับคงที่ 40/60 (ตั้งแต่ r3) เป็นการเลือกเชิงนำเสนอ (readability) ไม่ใช่ค่าที่
   คำนวณจากการกระจายของข้อมูลเหมือน Jenks เดิม — ถ้าถูกถามที่มาตัวเลขเจาะจง ต้องตอบตรงๆ
   ว่าเป็นเลขกลมที่ตรวจสอบแล้วว่าใกล้เคียงผลลัพธ์ Jenks ไม่ใช่มีทฤษฎีรองรับโดยตรง
