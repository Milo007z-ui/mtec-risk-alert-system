...
{
  // กรวยแคบ ±20° เตือนเฉพาะจุดที่เกือบตรงหน้ารถ (เลือกจากการจำลองขับถนนจริง)
  assert("ค่าเริ่มต้น = 20", FRONT_CONE_DEG === 20);
  const O = [13.7563, 100.5018];
  ...
  assert("ที่ 20° จุดตั้งฉาก 90° ถูกตัดออก",
    !isAhead(0, ...O, ...E, FRONT_CONE_DEG));
  // เหนือ 453 ม. ตะวันออก 230 ม. = เบน ~27° -> อยู่ใน ±30° เดิม แต่หลุด ±20°
  assert("จุดที่เบน ~27° -> ตัดออกที่ ±20° (±30° เดิมยังเตือน)",
    !isAhead(0, ...O, 13.76037, 100.50393, FRONT_CONE_DEG) && isAhead(0, ...O, 13.76037, 100.50393, 30));
  // เบน ~15° ยังต้องเตือน
  assert("จุดที่เบน 15° -> ยังอยู่ในกรวย ±20°",
    isAhead(0, ...O, 13.76066, 100.50302, FRONT_CONE_DEG));
