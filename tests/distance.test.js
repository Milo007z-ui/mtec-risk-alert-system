...
{
  // 90 องศาไม่ใช่ค่าที่จูนมา แต่คือการทดสอบ half-plane พอดีตัว: cos(มุม) > 0
  assert("ค่าเริ่มต้น = 90 (= การทดสอบ 'อยู่หน้าเส้นตั้งฉากกับหัวรถไหม')",
    FRONT_CONE_DEG === 90);
  const O = [13.7563, 100.5018];
  const E = [13.7563, 100.5118]; // ตะวันออก ~1.1 กม. = ตั้งฉากกับหัวรถพอดี
  assert("ที่ 90° จุดตั้งฉาก 90° ถือว่าอยู่ข้างหน้า (นับขอบเข้าด้วย)",
    isAhead(0, ...O, ...E, FRONT_CONE_DEG));
  assert("จุดที่เบน 91° ถูกตัดออก",
    !isAhead(0, ...O, 13.75612, 100.5118, FRONT_CONE_DEG));
}
