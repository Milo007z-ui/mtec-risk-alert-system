...
const GPS = (() => {
  ...
  // วิ่งบนถนน "บางปะอิน - แขวงรามอินทรา" (~10.5 กม.) ผ่านจุดเสี่ยง 22 จุดในรัศมี 600 ม.
  // ตั้งแต่รอบ v2568-r7 (จำแนกระดับด้วย Severity Index) เส้นทางนี้ผ่านเฉพาะระดับ
  // ต่ำ 20 จุดและปานกลาง 2 จุด ไม่มีระดับสูง — ต่างจากรอบก่อนที่ผ่านครบ 3 ระดับ
  // เพราะจุดที่ SI สูงกระจุกอยู่คนละบริเวณกับจุดที่เกิดเหตุถี่
  const MOCK_ROUTE_URL = "data/mock_route.geojson";

  // สำรอง: ถ้าโหลดไฟล์เส้นทางไม่ได้ ค่อยร้อยจุดเสี่ยงเป็นเส้นตรงแทน
  // (id ตามรอบ calibration v2568-r7 — เรียงตามลำดับบนถนนบางปะอิน-แขวงรามอินทรา
  //  zone_140 เป็นจุดระดับปานกลาง SI 1.09 ที่อยู่กลางเส้นทาง)
  const MOCK_ROUTE_IDS = ["zone_117", "zone_140", "zone_121"];

  // จุดเสี่ยงที่ "ยกเว้นเฉพาะโหมดจำลอง" — ใส่ id จุดที่อยู่คนละฝั่งเลน/แรมป์ ที่รถไม่ได้ขับผ่านจริง
  // (รอบ v2568-r7 ยังไม่พบจุดที่ต้องยกเว้น)
